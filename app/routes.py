...
@main.route("/market-purchases/new", methods=["GET", "POST"])
@admin_required
def market_purchase_new():
    batches = AggregationBatch.query.order_by(AggregationBatch.id.desc()).all()

    if request.method == "POST":
        aggregation_batch_id = _parse_int(request.form.get("aggregation_batch_id"))
        ...
        if aggregation_batch_id:
            ...
            if (batch.animal_type or "").strip() != animal_type:
                flash("Selected aggregation batch animal type does not match the market purchase animal type.", "danger")
                return redirect(request.url)

        elif create_new_batch:
            site_name = (request.form.get("site_name") or "").strip()
            date_received = _parse_date(request.form.get("date_received")) or date.today()
            ...
            flash("Select an existing aggregation batch or choose to create a new one.", "danger")
            return redirect(request.url)
        ...
        if not _commit_or_rollback("Create market purchase"):
            return redirect(request.url)

        flash("Market purchase created. Add purchase lines next.", "success")
        return redirect(url_for("main.market_purchase_detail", purchase_id=purchase.id))
...
@main.route("/market-purchases/<int:purchase_id>", methods=["GET", "POST"])
@admin_required
def market_purchase_detail(purchase_id):
    ...
    if request.method == "POST":
        ...
        unit_price = _parse_float(request.form.get("unit_price_kes"))
        ...
        line = MarketPurchaseLine(
            ...
            estimated_live_weight_per_head_kg=_parse_float(request.form.get("live_weight")),
            estimated_carcass_weight_per_head_kg=_parse_float(request.form.get("carcass_weight")),
            avg_age_months=_parse_int(request.form.get("avg_age_months")),
        ...
        if not _commit_or_rollback("Add purchase line"):
            return redirect(request.url)
...
@main.route("/market-purchases/<int:purchase_id>/lines/<int:line_id>/edit", methods=["POST"])
@admin_required
def market_purchase_edit_line(purchase_id, line_id):
    ...
    line.estimated_live_weight_per_head_kg = _parse_float(request.form.get("live_weight"))
    line.estimated_carcass_weight_per_head_kg = _parse_float(request.form.get("carcass_weight"))
    line.avg_age_months = _parse_int(request.form.get("avg_age_months"))
    line.weight_method = (request.form.get("weight_method") or "").strip() or None
    line.notes = (request.form.get("notes") or "").strip() or None

    if not _commit_or_rollback("Update purchase line"):
        return redirect(url_for("main.market_purchase_detail", purchase_id=purchase.id))
...
@main.route("/market-purchases/<int:purchase_id>/lines/<int:line_id>/delete", methods=["POST"])
@admin_required
def market_purchase_delete_line(purchase_id, line_id):
    ...
    if not _commit_or_rollback("Delete purchase line"):
        return redirect(url_for("main.market_purchase_detail", purchase_id=purchase.id))
...
@main.route("/market-purchases/<int:purchase_id>/expenses", methods=["POST"])
@admin_required
def market_purchase_add_expense(purchase_id):
    ...
    expense = MarketPurchaseExpense(
        ...
        amount=amount,
        incurred_date=_parse_date(request.form.get("incurred_date")) or date.today(),
    ...
    if not _commit_or_rollback("Add expense"):
        return redirect(url_for("main.market_purchase_detail", purchase_id=purchase.id))
...
@main.route("/market-purchases/<int:purchase_id>/expenses/<int:expense_id>/edit", methods=["POST"])
@admin_required
def market_purchase_edit_expense(purchase_id, expense_id):
    ...
    expense.incurred_date = _parse_date(request.form.get("incurred_date")) or expense.incurred_date
    ...
    if not _commit_or_rollback("Update expense"):
        return redirect(url_for("main.market_purchase_detail", purchase_id=purchase.id))
...
@main.route("/market-purchases/<int:purchase_id>/expenses/<int:expense_id>/delete", methods=["POST"])
@admin_required
def market_purchase_delete_expense(purchase_id, expense_id):
    ...
    if not _commit_or_rollback("Delete expense"):
        return redirect(url_for("main.market_purchase_detail", purchase_id=purchase.id))
