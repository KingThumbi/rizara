...
def aggregation_route(animal_type: str, template_add: str):
    ...
    def add_aggregation():
        ...
        if not selected_ids:
            flash(f"Select at least one {animal_label.lower()}.", "danger")
            return redirect(request.url)
        ...
        for raw_id in selected_ids:
            ...
            if animal.aggregation_batch_id:
                continue

            animal.live_weight_kg = _parse_float(request.form.get(f"weight_{raw_id}"))
            animal.purchase_price_per_head = _parse_float(request.form.get(f"price_{raw_id}"))
            animal.weight_method = request.form.get(f"method_{raw_id}") or "scale"
        ...
        flash(f"{animal_label} aggregation batch created ({attached} animals).", "success")
        return redirect(url_for("main.dashboard"))

    return add_aggregation


aggregation_route("goat", "aggregation/batch_add.html")
aggregation_route("sheep", "aggregation/batch_add.html")
aggregation_route("cattle", "aggregation/batch_add.html")

# ======================
# Market Purchases (LEGACY WRAPPERS ONLY)
# ======================
...
def processing_route(animal_type: str, template_add: str):
    ...
    Model = model_map.get(animal_type)
    relation_name = relation_map.get(animal_type)

    if Model is None or relation_name is None:
        raise ValueError(f"Unsupported animal type: {animal_type}")
    ...
    def add_processing():
        ...
        if not animals:
            flash(
                f"No eligible {animal_label.lower()} were found in the selected aggregation batch.",
                "danger",
            )
            return redirect(request.url)
        ...
        source_batch.locked_at = datetime.utcnow()

        if not _commit_or_rollback(f"Create {animal_label} processing batch"):
            return redirect(request.url)

        flash(
            f"{animal_label} processing batch created successfully from aggregation batch "
            f"#{source_batch.id} ({attached_count} animals).",
            "success",
        )
        return redirect(url_for("main.view_invoiceable_batch", batch_id=processing_batch.id))
